...
function Premium() {
    ...
    useEffect(() => {
        ...
        if (cancellMatch) {
            axios.get(`http://${window.location.hostname}:5000/paymentCancelled`, {
                headers: {
            ...
            if (userMatch) {
                axios.get(`http://${window.location.hostname}:5000/verifyPremium?token=${params.get('token')}`, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('SyncUp_Auth_Token')}`
                    }
                }).then(res => {
    ...
    const convertPointsToPremium = () => {
        if (userData.value.chatpoints >= 499) {
            axios.get(`http://${window.location.hostname}:5000/convertPointsToPremium`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('SyncUp_Auth_Token')}`
                }
            }).then(res => {
    ...
    return (
        <>
            <div className="preimumPageContainer">
                ...
                <p className='closeBtn' onClick={() => window.history.back()} >&times;</p>
...
const Bigbox = function (props) {
    ...
    const makePayment = async (plan) => {
        ...
        const response = await axios.post("http://localhost:5000/stripePaymentSession", { plan }, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('SyncUp_Auth_Token')}`
            }
        });
